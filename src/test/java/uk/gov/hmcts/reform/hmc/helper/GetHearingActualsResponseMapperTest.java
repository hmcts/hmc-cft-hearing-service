...
class GetHearingActualsResponseMapperTest {
    ...
    void toHearingsResponseWhenDataIsPresentForIndividualDetails() {
        ...
        assertEquals("hearingType", response.getHearingPlanned().getPlannedHearingType());
        assertEquals(LocalDateTime.parse("2000-08-10T12:20:00"), response.getHearingPlanned()
            .getPlannedHearingDays().get(0).getPlannedStartTime());
        assertEquals(LocalDateTime.parse("2000-08-10T12:20:00"), response.getHearingPlanned()
            .getPlannedHearingDays().get(0).getPlannedEndTime());
        assertEquals("reference", response.getHearingPlanned().getPlannedHearingDays()
            .get(0).getParties().get(0).getPartyID());
        assertEquals("role", response.getHearingPlanned().getPlannedHearingDays()
            .get(0).getParties().get(0).getPartyRole());
        ...
        assertEquals("hearingType", response.getHearingActuals().getHearingOutcome().getHearingType());
        assertEquals(true, response.getHearingActuals().getHearingOutcome().getHearingFinalFlag());
        assertEquals(HearingResultType.ADJOURNED, response.getHearingActuals().getHearingOutcome().getHearingResult());
        assertEquals("resultReason", response.getHearingActuals()
            .getHearingOutcome().getHearingResultReasonType());
        assertEquals(LocalDate.of(2000,01,01),
                     response.getHearingActuals().getHearingOutcome().getHearingResultDate());
        assertEquals(LocalDate.of(2000,01,01),
                     response.getHearingActuals().getActualHearingDays().get(0).getHearingDate());
        ...
        assertEquals(LocalDateTime.parse("2021-08-10T12:20:00"),
                     response.getHearingActuals().getActualHearingDays().get(0)
                         .getPauseDateTimes().get(0).getPauseStartTime());
        assertEquals(LocalDateTime.parse("2021-08-10T12:20:00"),
                     response.getHearingActuals().getActualHearingDays().get(0)
                         .getPauseDateTimes().get(0).getPauseEndTime());
        assertEquals(1, response.getHearingActuals().getActualHearingDays()
            .get(0).getActualDayParties().get(0).getActualPartyId());
        assertEquals("roleType", response.getHearingActuals().getActualHearingDays()
            .get(0).getActualDayParties().get(0).getPartyRole());
        assertEquals("partySubChannel", response.getHearingActuals().getActualHearingDays()
            .get(0).getActualDayParties().get(0).getPartyChannelSubType());
        assertEquals(false, response.getHearingActuals().getActualHearingDays()
            .get(0).getActualDayParties().get(0).getDidNotAttendFlag());
        assertEquals("1", response.getHearingActuals().getActualHearingDays()
            .get(0).getActualDayParties().get(0).getRepresentedParty());
